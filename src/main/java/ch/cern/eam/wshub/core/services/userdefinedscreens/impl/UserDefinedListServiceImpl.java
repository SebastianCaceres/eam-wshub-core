...
public class UserDefinedListServiceImpl implements UserDefinedListService {
    ...
    public List<UDLEntry> readUserDefinedListEntries(InforContext context, UDLEntryId property) throws InforException {
        ...
        List<Map<String, Object>> rows = userDefinedTableService.readUserDefinedTableRows(context, TABLE_NAME, filters, Arrays.asList("PVL_RENTITY", "PVL_CODE", "PVL_PROPERTY", "PVL_SEQNO", "PVL_VALUE", "PVL_DVALUE", "PVL_NVALUE"));
        try {
            return rows.stream().// sorted based on property first, then sequence number
            sorted((a, b) -> {
            ...
            // in case this::getUDLEntryFromMap fails
            throw generateFault(e.getMessage());
        }
    }
    ...
    public String updateUserDefinedListEntry(InforContext context, UDLEntry entry) throws InforException {
        ...
        tools.log(Level.SEVERE, "UserDefinedListServiceImpl::updateUserDefinedListEntry updated more than 1 row");
    ...
    public void writeUDLToEntityCopyFrom(InforContext context, UserDefinedListHelpable entity, EntityId entityId) {
        try {
            ...
            if (entries != null) {
                ...
                Map<String, ArrayList<UDLValue>> copyFromEntries = readUserDefinedLists(context, new UDLEntryId(new EntityId(entityId.getEntityType(), entity.getCopyFrom())));
    ...
    public void writeUDLToEntity(InforContext context, UserDefinedListHelpable entity, EntityId entityId) {
        try {
            if (entity.getUserDefinedList() != null) {
                setUserDefinedLists(context, entityId, entity.getUserDefinedList());
            }
        } catch (Exception e) {
