...
public class ScreenLayoutServiceImpl implements ScreenLayoutService {
    ...
    public ScreenLayoutServiceImpl(ApplicationData applicationData, Tools tools) {
    ...
    public ScreenLayout readScreenLayout(InforContext context, String systemFunction, String userFunction, List<String> tabs, String userGroup, String entity) throws InforException {
        try {
            ...
            Function<String, ScreenLayout> loader = key -> loadScreenLayout(context, systemFunction, userFunction, tabs, userGroup, entity);
            return Optional.ofNullable(InforClient.cacheMap.get(CacheKey.SCREEN_LAYOUT)).map(cache -> (ScreenLayout) cache.get(screenLayoutCacheKey, loader)).orElseGet(() -> loader.apply(screenLayoutCacheKey));
    ...
    public ScreenLayout loadScreenLayout(InforContext context, String systemFunction, String userFunction, List<String> tabs, String userGroup, String entity) {
        try {
            ...
            screenLayout.getFields().values().forEach(e -> bindUdfDescription(udfDetails.get(e.getElementId()), e));
            // Add other tabs
    ...
    public List<Map<String, String>> getGenericLov(InforContext context, GenericLov genericLov) throws InforException {
        ...
        GridRequestFilter filter = new GridRequestFilter(listReturnFields.get(0), genericLov.getHint().toUpperCase(), genericLov.isExact() ? "EQUALS" : "BEGINS", searchOnDesc || searchOnAlias ? GridRequestFilter.JOINER.OR : GridRequestFilter.JOINER.AND);
        gridRequest.getGridRequestFilters().add(filter);
        if (searchOnDesc) {
            Arrays.stream(genericLov.getHint().split(" ")).forEach(name -> {
                gridRequest.addFilter("description", " " + name, "CONTAINS", GridRequestFilter.JOINER.OR, true, false);
                gridRequest.addFilter("description", name, "BEGINS", GridRequestFilter.JOINER.AND, false, true);
            });
        }
    ...
    private Map<String, Tab> getAllTabs(InforContext context, String userGroup, String userFunction) throws InforException {
        ...
        // Artificial filter to improve performance
        gridRequest.addFilter("tabcode", "", "BEGINS");
    ...
    private URLParam rowToURLParam(GridRequestRow row) {
        return new URLParam(GridTools.getCellContent("parametername", row), GridTools.getCellContent("parametervalue", row), Objects.equals(GridTools.getCellContent("system", row), "true"), Objects.equals(GridTools.getCellContent("usefieldvalue", row), "true"));
    }
    ...
    private Map<String, ElementInfo> getTabLayout(String entity, Map<String, Map<String, String>> defaultLayout, Map<String, Map<String, String>> layout) throws InforException {
        ...
        elements.stream().filter(element -> element.getXpath() != null && !element.getXpath().isEmpty()).forEach(element -> element.setXpath("EAMID_" + element.getXpath().replace("\\", "_")));
    ...
    private Map<String, String> getTabLayoutLabels(InforContext context, String userFunction, String systemFunction) throws InforException {
        try {
            ...
            return Optional.ofNullable(InforClient.cacheMap.get(CacheKey.SCREEN_LAYOUT_LABEL)).map(cache -> (Map<String, String>) cache.get(screenLayoutLabelCacheKey, loader)).orElseGet(() -> loader.apply(screenLayoutLabelCacheKey));
        } catch (RuntimeException e) {
            if (e.getCause() instanceof InforException) {
                throw (InforException) e.getCause();
            }
    ...
    private Map<String, UserDefinedFieldDescription> getUdfDetails(InforContext context, String entity) throws InforException {
        ...
        Map<String, UserDefinedFieldDescription> result = convertGridResultToMap(UserDefinedFieldDescription.class, "udf_field", null, gridsService.executeQuery(context, gridRequest));
