...
public class GridsServiceImpl implements GridsService {
    ...
    public GridsServiceImpl(ApplicationData applicationData, Tools tools, GridDataspyRepository gridDataspyRepository, GridFieldRepository gridFieldRepository, GridMetadataRequestResultRepository gridMetadataRequestResultRepository, InstallParametersRepository installParametersRepository) {
    ...
    public GridMetadataRequestResult getGridMetadataInfor(InforContext context, String gridName, String gridId) {
        ...
        return Optional.ofNullable(InforClient.cacheMap.get(CacheKey.GRID_ID)).map(cache -> (GridMetadataRequestResult) cache.get(gridIdCacheKey, loader)).orElseGet(() -> loader.apply(gridIdCacheKey));
    }
