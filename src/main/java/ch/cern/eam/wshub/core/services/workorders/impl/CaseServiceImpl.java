...
public class CaseServiceImpl implements CaseService {
    ...
    public InforCase readCase(InforContext context, String caseID) throws InforException {
        ...
        MP3643_GetCaseManagement_001_Result result = tools.performInforOperation(context, inforws::getCaseManagementOp, getCase);
        ...
        if (caseManagement.getTrackingDetails() != null) {
            caseMT.setScheduledStartDate(tools.getDataTypeTools().decodeInforDate(caseManagement.getTrackingDetails().getSCHEDULEDSTARTDATE()));
            caseMT.setScheduledEndDate(tools.getDataTypeTools().decodeInforDate(caseManagement.getTrackingDetails().getSCHEDULEDENDDATE()));
            caseMT.setRequestedStartDate(tools.getDataTypeTools().decodeInforDate(caseManagement.getTrackingDetails().getREQUESTEDSTART()));
