...
public class WorkOrderServiceImpl implements WorkOrderService {
	...
	public WorkOrder readWorkOrder(InforContext context, String number) throws InforException {
		...
		return workOrderRepository.findById(woNumber)
	...
	*/
	public String createWorkOrder(InforContext context, WorkOrder workorderParam) throws InforException {
		...
		if (workorderParam.getCopyFrom() != null) {
			...
			WorkOrder source = workOrderRepository.findById(sourceNumber)
					.orElseThrow(() -> tools.generateFault("Source work order not found for copyFrom: " + sourceNumber));
		...
		if (workorderParam.getStandardWO() != null && !workorderParam.getStandardWO().trim().isEmpty()) {
			...
			if (workorderParam.getDescription() == null)       workorderParam.setDescription(swo.getDesc());
			if (workorderParam.getClassCode() == null)         workorderParam.setClassCode(swo.getWoClassCode());
			if (workorderParam.getPriorityCode() == null)      workorderParam.setPriorityCode(swo.getPriorityCode());
			if (workorderParam.getTypeCode() == null)          workorderParam.setTypeCode(swo.getWorkOrderTypeCode());
			if (workorderParam.getProblemCode() == null)       workorderParam.setProblemCode(swo.getProblemCode());
			if (workorderParam.getCustomFields() == null)      workorderParam.setCustomFields(swo.getCustomFields());
			if (workorderParam.getUserDefinedFields() == null) workorderParam.setUserDefinedFields(swo.getUserDefinedFields());
		}
	...
	public String updateWorkOrderStatus(InforContext context, String workOrderNumber, String statusCode) throws InforException {
		String woNumber = extractEntityCode(workOrderNumber);
		WorkOrder wo = workOrderRepository.findById(woNumber)
				.orElseThrow(() -> tools.generateFault("Work order not found: " + woNumber));
	...
	private void mergeFromSource(WorkOrder source, WorkOrder target) {
		...
		if (target.getClassCode() == null)          target.setClassCode(source.getClassCode());
		if (target.getPriorityCode() == null)       target.setPriorityCode(source.getPriorityCode());
		if (target.getTypeCode() == null)           target.setTypeCode(source.getTypeCode());
		if (target.getProblemCode() == null)        target.setProblemCode(source.getProblemCode());
		if (target.getEquipmentCode() == null)      target.setEquipmentCode(source.getEquipmentCode());
		if (target.getDepartmentCode() == null)     target.setDepartmentCode(source.getDepartmentCode());
		if (target.getStatusCode() == null)         target.setStatusCode(source.getStatusCode());
		if (target.getCustomFields() == null)       target.setCustomFields(source.getCustomFields());
		if (target.getUserDefinedFields() == null)  target.setUserDefinedFields(source.getUserDefinedFields());
